...
export const FabricacionForm = () => {
  ...
  const handleGenerateLote = async () => {
    ...
    if (cat) {
      const nuevoLote = await generateCode(cat.prefijo, 'fabricaciones');
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <form className="bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-xl space-y-6">
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                >
                  ...
                  {categorias?.map(c => <option key={c.id} value={c.id}>{c.nombre} ({c.prefijo})</option>)}
                </select>
                <button 
                  type="button"
          ...
          <div className="space-y-4">
            <div>
              ...
              >
                ...
                {basesDisponibles?.map(b => <option key={b.codigo} value={b.codigo}>{b.codigo} - {b.proveedor}</option>)}
              </select>
            </div>
        ...
        >
          REGISTRAR FABRICACIÓN (GENERAR LOTE)
        </button>
      </form>
    </div>
  );
